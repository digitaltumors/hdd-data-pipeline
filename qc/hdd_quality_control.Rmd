...
with missing values representing compounds that were not measured. HDD v1.1
also carries assay-level metadata in the Bioassays `rowData`, including assay
name, source name, source ID, target name, and target protein accession.

```{r bioassays-preview}
se <- experiments(hdd_mae)[["Bioassays"]]
assay_mat <- assay(se)
bioassay_rowdata <- as.data.frame(rowData(se)) |>
...
c(
  bioassay_metadata_rows = nrow(bioassay_rowdata),
  bioassay_metadata_cols = ncol(bioassay_rowdata),
  assays_with_target_name = sum(!is.na(bioassay_rowdata$target_name) & nzchar(trimws(bioassay_rowdata$target_name))),
  assays_with_target_protein_accession = sum(!is.na(bioassay_rowdata$target_protein_accession) & nzchar(trimws(bioassay_rowdata$target_protein_accession)))
)
...
ggplot(
  bioassay_rowdata |>
    count(source_name, sort = TRUE),
  aes(x = reorder(source_name, n), y = n)
) +
  geom_col(fill = "#72B7B2") +
  coord_flip() +
  labs(
    title = "PubChem Bioassays assays by source",
    x = "Source name",
    y = "Assay count"
  ) +
  theme_minimal()
