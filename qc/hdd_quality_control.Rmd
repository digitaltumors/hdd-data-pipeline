---
title: "Harmonized Drug Dataset Version 1.1 (HDD_v1.1)"
...
This report is a quality control walkthrough for the Harmonized Drug Dataset Version 1.1 (HDD_v1.1)
...
- **BindingDB**: currently deprecated for the active HDD_v1.1 build due to scope changes driven by data quality concerns.
...
hdd_v1_1 <- readRDS("data/results/HDD_v1.1.RDS")
hdd_v1_1
```
...
```{r mae-snapshot}
experiment_names <- names(experiments(hdd_v1_1))
experiment_names
...
experiment_summary <- do.call(
  rbind,
  lapply(experiments(hdd_v1_1), summarize_experiment)
)
...
all_in_coldata <- lapply(experiments(hdd_v1_1), function(se) {
  all(colnames(se) %in% rownames(colData(hdd_v1_1)))
})
...
missing_samples <- lapply(experiments(hdd_v1_1), function(se) {
  setdiff(colnames(se), rownames(colData(hdd_v1_1)))
})
...
Below, each experiment in the HDD_v1.1 MAE is reviewed.

## BindingDB

BindingDB is currently deprecated for the active HDD_v1.1 build. If a future MAE
includes it again, this section will summarize the compound-by-target affinity
matrix derived from BindingDB.

```{r bindingdb-preview}
if (!has_experiment("BindingDB")) {
  cat("BindingDB is not present in this HDD_v1.1 build.\n")
} else {
  se <- experiments(hdd_v1_1)[["BindingDB"]]
  assay_mat <- assay(se)
...
```{r bindingdb-qc}
if (!has_experiment("BindingDB")) {
  cat("BindingDB QC is skipped because the experiment is absent.\n")
} else {
...
if (!has_experiment("BindingDB")) {
  cat("BindingDB plots are skipped because the experiment is absent.\n")
} else {
...
BindingDB commentary is omitted for the current build because the experiment is
not part of the active MAE.
...
```{r bioassays-preview}
se <- experiments(hdd_v1_1)[["Bioassays"]]
assay_mat <- assay(se)
...
```{r toxcast-preview}
se <- experiments(hdd_v1_1)[["ToxCast"]]
...
```{r clintox-preview}
se <- experiments(hdd_v1_1)[["ClinTox"]]
assay_mat <- assay(se)
...
for (fp_name in fp_names) {
  se <- experiments(hdd_v1_1)[[fp_name]]
  assay_mat <- assays(se)[[1]]
...
```{r moa-counts}
col.Data <- colData(hdd_v1_1) %>% as.data.frame() 
...
if (is.na(selected_fp_name)) {
  stop("No Morgan fingerprint experiments are available in HDD_v1.1.")
}

fps <- assay(experiments(hdd_v1_1)[[selected_fp_name]])
