---
title: "Harmonized Drug Dataset Version 1 (HDD_v1)"
...
This report is a quality control walkthrough for the Harmonized Drug Dataset Version 1 (HDD_v1)
...
- **BindingDB**: currently deprecated for the active HDD_v1 build due to scope changes driven by data quality concerns.
...
hdd_v1 <- readRDS("data/results/HDD_v1.RDS")
hdd_v1
```
...
```{r mae-snapshot}
experiment_names <- names(experiments(hdd_v1))
experiment_names
...
experiment_summary <- do.call(
  rbind,
  lapply(experiments(hdd_v1), summarize_experiment)
)
...
all_in_coldata <- lapply(experiments(hdd_v1), function(se) {
  all(colnames(se) %in% rownames(colData(hdd_v1)))
})
...
missing_samples <- lapply(experiments(hdd_v1), function(se) {
  setdiff(colnames(se), rownames(colData(hdd_v1)))
})
...
Below, each experiment in the HDD_v1 MAE is reviewed.

## BindingDB

BindingDB is currently deprecated for the active HDD_v1 build. If a future MAE
...
if (!has_experiment("BindingDB")) {
  cat("BindingDB is not present in this HDD_v1 build.\n")
} else {
  se <- experiments(hdd_v1)[["BindingDB"]]
  assay_mat <- assay(se)
...
```{r bioassays-preview}
se <- experiments(hdd_v1)[["Bioassays"]]
assay_mat <- assay(se)
...
```{r moa-counts}
col.Data <- colData(hdd_v1) %>% as.data.frame() 
...
if (is.na(selected_fp_name)) {
  stop("No Morgan fingerprint experiments are available in HDD_v1.")
}

fps <- assay(experiments(hdd_v1)[[selected_fp_name]])
